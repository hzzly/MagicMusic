  ...
  <div class="play">
    <div class="content">
      <div class="head">
        ...
        <div class="h-icon" @click="showToast">
          <i class="icon">&#xe93b;</i>
        </div>
      </div>
      <div class="lyrics">
        <div class="m-name">{{audio[0].name}}</div>
        <div class="s-name" v-if="audio[0].ar">{{audio[0].ar[0].name}}</div>
        <div class="s-name" v-else>{{audio[0].artists[0].name}}</div>
        <div class="lyric">
      ...
      <div class="control">
        <div>
          <div class="loop" @click="showToast">
            <i class="icon">&#xe819;</i>
          </div>
          <div class="pre" @click="_pre()">
            <i class="icon">&#xe61e;</i>
          </div>
          <div class="pause">
            <i class="icon" v-if="playing" @click="_pause()">&#xe644;</i>
            <i class="icon pause-icon" v-else @click="_play()">&#xe630;</i>
          </div>
          <div class="next" @click="_next()">
            <i class="icon">&#xe604;</i>
          </div>
      ...
      <div class="process" @click="showToast">
        ...
        <div
          class="pro"
          :style="{transform: `translateX(${translateX}) rotate(${deg*1 + 56.5*((now / size).toFixed(3))}deg)`}"
        ></div>
      </div>
    </div>
    <div class="background">
      <img :src="audio[0].al.picUrl" width="100%" height="100%" alt v-if="audio[0].al">
      <img :src="audio[0].album.picUrl" width="100%" height="100%" alt v-else>
    </div>
  </div>
</template>
...
.play {
  ...
  .content {
    ...
    .head {
      ...
      border-bottom: 1px solid rgba(40, 34, 78, 0.2);
      .h-icon {
        width: px2rem(100px);
        cursor: pointer;
        .icon {
